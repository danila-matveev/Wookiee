...
function renderCell(key: string, a: ArtikulRow): React.ReactNode {
  switch (key) {
    case "artikul":
      return <span className="font-mono text-xs text-primary">{a.artikul}</span>
    case "model":
      return (
        <div className="flex flex-col">
          ...
          {a.nazvanie_etiketka && (
            <span className="text-[11px] text-muted">{a.nazvanie_etiketka}</span>
          )}
        </div>
      )
    case "cvet":
      return (
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-xs text-secondary">{a.cvet_color_code ?? "—"}</span>
          {a.cvet_nazvanie && <span className="text-muted text-xs">{a.cvet_nazvanie}</span>}
        </div>
      )
    case "status":
    ...
    case "ozon_art":
      return (
        <span className="font-mono text-[11px] text-secondary">{a.artikul_ozon ?? "—"}</span>
      )
    case "created":
      return <span className="text-xs text-muted">{relativeDate(a.created_at)}</span>
    case "updated":
      return <span className="text-xs text-muted">{relativeDate(a.updated_at)}</span>
    case "kategoriya":
      return <span className="text-xs text-secondary">{a.kategoriya ?? "—"}</span>
    case "kollekciya":
      return <span className="text-xs text-secondary">{a.kollekciya ?? "—"}</span>
    case "fabrika":
      return <span className="text-xs text-secondary">{a.fabrika ?? "—"}</span>
    default:
      return null
  }
}
...
export function ArtikulyPage() {
  ...
  if (isLoading) {
    return <div className="px-6 py-8 text-sm text-label">Загрузка артикулов…</div>
  }
  if (error) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-3 shrink-0">
        ...
        <h1 className="text-3xl text-primary font-serif italic">Артикулы</h1>
      ...
      <div className="px-6 pb-3 flex items-center gap-2 flex-wrap shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 text-sm border border-default rounded-md bg-surface outline-none focus:border-[var(--color-border-strong)] w-80"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        <button
          ...
          className={`px-2 py-1 text-xs rounded-md transition-colors ${
            statusFilter === "all" ? "bg-elevated text-white" : "text-secondary hover:bg-surface-muted"
          }`}
        >
        ...
        {artikulStatuses.map((s) => (
          <button
            ...
            className={`px-2 py-1 text-xs rounded-md transition-colors ${
              statusFilter === s.id ? "bg-elevated text-white" : "text-secondary hover:bg-surface-muted"
            }`}
          >
            {s.nazvanie}
