...
// FormOutput is used only for the submit handler cast; resolver v3 uses FormInput throughout
...
export function IntegrationEditDrawer({
  ...
  const onSubmit: SubmitHandler<FormInput> = async (values) => {
    // After zod parsing, values are actually FormOutput — cast for type safety
    const parsed = values as unknown as FormOutput;
    const body: IntegrationInput = {
      blogger_id: parsed.blogger_id,
      marketer_id: parsed.marketer_id,
      publish_date: parsed.publish_date,
      channel: parsed.channel,
      ad_format: parsed.ad_format,
      marketplace: parsed.marketplace,
      stage: parsed.stage,
      outcome: parsed.outcome,
      is_barter: parsed.is_barter,
      cost_placement: parsed.cost_placement,
      cost_delivery: parsed.cost_delivery,
      cost_goods: parsed.cost_goods,
      erid: parsed.erid,
      notes: parsed.notes,
      theme: parsed.theme,
      audience_age: parsed.audience_age,
      subscribers: parsed.subscribers,
      min_reach: parsed.min_reach,
      engagement_rate: parsed.engagement_rate,
      fact_views: parsed.fact_views,
      fact_cpm: parsed.fact_cpm,
      fact_clicks: parsed.fact_clicks,
      fact_ctr: parsed.fact_ctr,
      fact_cpc: parsed.fact_cpc,
      fact_carts: parsed.fact_carts,
      cr_to_cart: parsed.cr_to_cart,
      fact_orders: parsed.fact_orders,
      cr_to_order: parsed.cr_to_order,
      fact_revenue: parsed.fact_revenue,
      contract_url: parsed.contract_url,
      post_url: parsed.post_url,
      tz_url: parsed.tz_url,
      screen_url: parsed.screen_url,
      post_content: parsed.post_content,
      analysis: parsed.analysis,
      recommended_models: parsed.recommended_models,
      has_marking: parsed.has_marking,
      has_contract: parsed.has_contract,
      has_deeplink: parsed.has_deeplink,
      has_closing_docs: parsed.has_closing_docs,
      has_full_recording: parsed.has_full_recording,
      all_data_filled: parsed.all_data_filled,
      has_quality_content: parsed.has_quality_content,
      complies_with_rules: parsed.complies_with_rules,
    };
