...
import { z } from 'zod';
import type { BloggerDetailOut, BloggerInput, BloggerStatus } from '@/api/crm/bloggers';
import { useBlogger, useUpsertBlogger } from '@/hooks/crm/use-bloggers';
...
export function BloggerEditDrawer({ open, onClose, bloggerId }: BloggerEditDrawerProps) {
  ...
  const onSubmit: SubmitHandler<FormInput> = async (values) => {
    ...
    const body: BloggerInput = {
      display_handle: parsed.display_handle,
      real_name: parsed.real_name,
      status: parsed.status,
      default_marketer_id: parsed.default_marketer_id,
      price_story_default: parsed.price_story_default,
      price_reels_default: parsed.price_reels_default,
      contact_tg: parsed.contact_tg,
