...
# Пути к корню проекта и скриптам
PROJECT_ROOT = Path(__file__).resolve().parent.parent
LOCALIZATION_DIR = PROJECT_ROOT / "MP scripts" / "Index_localization WB"

# Добавляем пути для импорта
sys.path.insert(0, str(PROJECT_ROOT))
sys.path.insert(0, str(LOCALIZATION_DIR))

from wb_sheets_sync.config import CABINET_IP, CABINET_OOO
...
from generate_localization_report_v3 import (
    run_analysis,
    load_barcodes,
    load_statuses,
    BASE_PATH,
...
from wb_localization_mappings import log_unknown_mappings

from vasily_agent.history import History
...
class VasilyService:
    ...
    def run_report(self, cabinet: str, days: int = 30, **kwargs) -> dict:
        ...
        # 1. Общие данные
        sku_db = str(PROJECT_ROOT / "wookiee_sku_database" / "Спецификации.xlsx")
