...
1. **Bearer JWT** (для Hub frontend)
   ...
   Верификация через **JWKS endpoint Supabase** (`{SUPABASE_URL}/auth/v1/.well-known/jwks.json`):
   - текущий ключ проекта — **ES256** (ECC P-256, асимметричный)
   - PyJWKClient автоматически получает публичный ключ по `kid` из JWT-header
   - проверяется подпись + `audience=authenticated`
   - в `algorithms=` оставлены `["ES256", "RS256", "HS256"]` для обратной совместимости со старыми токенами
...
SUPABASE_URL=https://<project>.supabase.co     # для JWKS-верификации Bearer JWT
...
`SUPABASE_JWT_SECRET` (legacy HS256 shared-secret) больше **не нужен** — Supabase перешёл на ES256, верификация идёт через JWKS endpoint автоматически.
...
1. **margin_before_ads формула** (TODO в `aggregate_to_weeks`):
...
2. **Charlotte и другие модели без `artikul_modeli`** — не появятся в дропдауне, пока поле в Supabase не заполнят.

3. **OZON не поддерживается** — Phase 1 только WB. Селектор маркетплейса в UI пока не активен.

4. **Источник данных по блогерам — Google Sheets**, не Supabase CRM.
...
5. **buyout_pct — лаговый показатель** (3-21 дн. задержка). В UI отображается как информационный, не как причина изменения маржи.

6. **Остаточное расхождение opens vs PowerBI ~20%** — методология DAX отличается от наших SQL-сумм. Принимаем как известное расхождение; внутренне все CR консистентны (intra-source). См. `docs/database/DATA_QUALITY_NOTES.md` п.4.
