...
_QUALITY_STANDARDS = """СТАНДАРТЫ КАЧЕСТВА SUMMARY (это главное, на что смотрит команда после встречи):

— Встречи Wookiee длятся 1-2 часа и содержат МНОГО тем. Один-два пункта topics — это сигнал, что ты ленишься. Норма: 5-15 тем для часа+ встречи.

— topics: каждая тема — содержательная фраза по сути ("повышение СПП на Wendy до 35% с 15.05", "редизайн упаковки Moon под жалобы на доставку OZON", "тест 3 новых креативов на блогерах с бюджетом 50К"). НЕ "обсуждение продаж", НЕ "разное", НЕ "вопросы команды". anchor — "[MM:SS]" от старта записи, момент когда тема была поднята.

— decisions: ВСЕ конкретные зафиксированные решения, развёрнутыми предложениями. "Решили запустить X с условием Y", "Отказались от Z потому что W". Не одно слово, не "обсудили". Если за встречу не приняли ни одного решения — пустой массив (это редко, но бывает).

— tasks: РАЗВЁРНУТЫЕ задачи в формате полного брифа. Каждая:
  * assignee — реальное имя ответственного из Participants (или "—" если в речи не назначили).
  * what — ПОЛНОЕ описание задачи в 1-3 предложения. НЕ "сделать карточку", а "переписать описание карточки Wendy с акцентом на состав ткани и сертификаты — текущая версия не упоминает гипоаллергенность, что критично для целевой аудитории". Если в речи прозвучали конкретные параметры (цвета, цены, бюджеты, размеры партий, требования к упаковке, ссылки на референсы) — обязательно включай их.
  * when — срок если он прозвучал явно ("до пятницы", "к 20.05", "после съёмки 18-го") или null. Не выдумывай.
  * context — короткое объяснение зачем эта задача (1 предложение): какая проблема решается, какая гипотеза проверяется, какой бизнес-эффект ожидается. Опускай поле или null, если в речи контекста не было.
  * conditions — условности, которые проговорили ("только если ткань придёт до 25.05", "при условии что блогер согласится на наш ТЗ", "после согласования бюджета с Данилой"). Опускай или null если условий не было.

— participants: реальные имена из Participants/speakers_map. БЕЗ "Speaker N" в финальном summary.
"""
...
_PROMPT_TEMPLATE = """Ты — старший аналитик расшифровок встреч бренда Wookiee (нижнее бельё на Wildberries и OZON). На вход — сегменты ASR (Yandex SpeechKit) с диаризацией и список участников. Твоя задача — превратить запись 1-2 часовой бизнес-встречи в применимый рабочий артефакт, по которому команда может действовать без переслушивания. Верни строго один JSON-объект, без любого текста до или после.
...
3) Tags — 1-6 наиболее релевантных из канонического списка: {tags}.

4) Summary — главное артефакт.
...
_PROMPT_SUMMARY_ONLY = """Ты — старший аналитик расшифровок встреч бренда Wookiee (нижнее бельё на Wildberries и OZON). На вход — сегменты ASR с диаризацией и список участников. Это chunked-режим: тебе НЕ нужно собирать paragraphs (это отдельный вызов). Верни СТРОГО один JSON следующей формы (никаких markdown, никаких комментариев):
