...
| Таблица | Источник API | Данные | Уникальный ключ |
|---------|-------------|--------|-----------------|
| wb.wb_adv | /api/v2/fullstats | Показы, клики, расход, корзины, заказы, CTR, CPC, CR | (date, nmid, advertid, lk) |
| wb.content_analysis | /api/v2/nm-report/detail | Органическая воронка: показы карточки, корзины, заказы, выкупы | (date, nmid, lk) |

**Производные метрики (вычисляются агентом Олег, НЕ хранятся в БД):**
CPM, CPL, CPO, конверсии по шагам воронки, ДРР

**Проверки качества:**
- Свежесть: wb_adv и content_analysis имеют данные за вчера
- Кросс-валидация: `SUM(wb_adv.sum)` ~ `SUM(abc_date.reclama)` (порог < 5%)
- Полнота: нет пропущенных дат в рекламных таблицах за 30 дней
