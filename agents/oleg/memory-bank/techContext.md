...
- **DeepElasticityService** — поартикульный анализ заказов с сегментацией SKU по статусам ("Развитие" vs "Ликвидация"), First-Sale Alignment и синтетическая агрегация спроса.
...
1. **Oleg Agent Runner (`agent_runner.py`)**:
   ...
   - Мониторит свежесть данных (`DataFreshnessService`): проверка на наличие 50% выручки (vs 7d avg), положительной маржинальности и логистики (`logist`/`logist_end`), а также кросс-чек `abc_date` vs `orders`.
...
- `get_deep_price_analysis` — Глубокий анализ эластичности по группам SKU: 'Развитие' (Продается, Новый, Запуск) и 'Ликвидация' (Выводим). Использует поартикульные данные заказов и цены после СПП.
