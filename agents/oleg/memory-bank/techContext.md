...
- **pandas >= 2.0.0**
- **scipy >= 1.12.0**
- **statsmodels >= 0.14.1** — регрессионный анализ, эластичность
...
1. **Oleg Agent Runner (`agent_runner.py`)**:
   ...
   - Мониторит свежесть данных (`DataFreshnessService`).
...
- `shared/data_layer.py` — все SQL-запросы (WB, OZON, Supabase)
...
### Ценовые (9 tools)
- `get_price_elasticity` — ценовая эластичность (log-log OLS)
