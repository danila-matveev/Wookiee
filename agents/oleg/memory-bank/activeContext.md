...
### 2026-02-19 (Advertising & Glossary Refinement)
- **Консолидация анализа рекламы**: Таблицы детального рекламного анализа перенесены из раздела 10.8 в раздел 8.2 (Detailed Report), дубликаты удалены.
- **Drill-down по рекламе**: В таблицу 6.4 добавлены метрики заказов (шт/руб), ДРР (от заказов/продаж) и ROMI вместо CPO/CPA.
- **Внешняя реклама**: Рефакторинг раздела 10.4 в компактную матрицу системности каналов (Блогеры vs VK/Яндекс).
- **Терминология**:
    - **Маржинальность**: Относительная доходность продаж (Unit Economics).
    - **Рентабельность**: Чистая прибыль компании (Net Profit), цель 15%.
    - **Шкала маржинальности**: Унифицирована (4 зоны: >25% Рост, 20-25% Норма, 15-20% Внимание, <15% Критично).
- **НДС**: В таблице Reconciliation явно указано, что НДС — технический фактор, зависящий от СПП.
