...
→ ДАННЫЕ УЖЕ ОБЪЕДИНЕНЫ ПО model_osnova backend-ом (Set Vuki, Vuki, Moon и т.д.).
→ Используй эти названия (model_osnova) ДЛЯ ВСЕХ ВЫВОДОВ, СВОДОК И ТАБЛИЦ. Не пытайся дробить их на VukiW, RubyP и т.д.
→ Определи ДРАЙВЕРЫ прибыли: топ-5 моделей по РОСТУ маржи в ₽
→ Определи АНТИ-ДРАЙВЕРЫ: модели с ДРР > 30% ИЛИ маржа < 0 ИЛИ маржа упала > 30%
→ Обязательно выводи данные по моделям с отрицательной маржой! Это критически важно для поиска потерь.
...
- **Общие выводы / Сводка / Драйверы:** использовать ТОЛЬКО официальные имена `model_osnova` возвращаемые инструментами (Vuki, Moon, Set Vuki, Ruby, Joy, Wendy, Bella, Audrey и т.д.). НЕ игнорируй ни одну модель, если она есть в данных!
- Максимум 45 строк
