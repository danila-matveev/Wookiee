...
→ Определи ДРАЙВЕРЫ прибыли: топ-3 моделей по РОСТУ маржи в ₽
→ Определи АНТИ-ДРАЙВЕРЫ: модели с ДРР > 30% ИЛИ маржа < 0 ИЛИ маржа упала > 30%
→ Обязательно выводи данные даже по моделям с отрицательной маржой! Это критически важно для поиска потерь.
→ В финальном отчёте: «Драйверы: Wendy (+46 тыс), Ruby (+25 тыс). Анти-драйверы: Joy (убыток -5.9 тыс, ДРР 41%)»
...
- **Общие выводы / Сводка / Драйверы:** использовать ТОЛЬКО `model_osnova` (Vuki, Moon, Set Vuki, Wendy).
- Максимум 45 строк
