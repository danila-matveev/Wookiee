...
→ ЦЕНОВАЯ СТРАТЕГИЯ:
   - СПП изменился > 2 п.п.? → указать влияние на комиссию и спрос
   - Если СПП вырос → рекомендация: поднять базовую цену на величину роста СПП
...
→ Суммировать маржу по model_osnova, НЕ по артикулам
→ Определи ДРАЙВЕРЫ прибыли: топ-3 model_osnova по РОСТУ маржи в ₽
→ Определи АНТИ-ДРАЙВЕРЫ: model_osnova с ДРР > 30% ИЛИ маржа < 0 ИЛИ маржа упала > 30%
→ Drill-down: если model_osnova аномальна → указать какие модели/артикулы внутри
→ В финальном отчёте: «Драйверы: Wendy (+46 тыс), Ruby (+25 тыс). Анти-драйверы: Joy WB (убыток -5.9 тыс, ДРР 41%)»
...
- Модели называть по model_osnova (Vuki, Moon, Ruby), НЕ по артикулам
- Максимум 45 строк
...
- ОБЯЗАТЕЛЬНО: разбивка по каналам WB/OZON
