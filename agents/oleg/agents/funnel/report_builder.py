...
def _build_recommendations(models: list[dict], brand: dict) -> str:
    ...
    if cro_drops:
        ...
        lines.append("**ГИПОТЕЗА:** Возможно, привлечён нецелевой трафик по новым ключевым словам, изменились позиции по нерелевантным запросам, или проблемы с контентом карточек (фото, описания, размерная сетка).")
        target_cro = cro_p * 0.9  # realistic target
        extra_orders = int(opens * (target_cro - cro_c) / 100)
        lines.append("**ДЕЙСТВИЕ:** Проверить ключевые слова на релевантность. Аудит карточек с падающим CRO. Проверить наличие размеров у проблемных артикулов. Проверить отзывы на негатив.")
    ...
    if traffic_drops:
        ...
        lines.append("**ГИПОТЕЗА:** Снижение позиций в поиске по ключевым словам, сезонный спад, или снижение рекламной активности.")
        lines.append("**ДЕЙСТВИЕ:** Проверить позиции по топ-ключевым словам. Обновить заголовки и описания карточек. Рассмотреть увеличение рекламы на артикулы с высоким CR.")
    ...
    if growers:
        ...
        lines.append("**ГИПОТЕЗА:** Эти артикулы попали в ТОП по высокочастотным запросам или получили вирусный эффект.")
        lines.append("**ДЕЙСТВИЕ:** Проверить ключевые слова, по которым эти артикулы находятся. Рассмотреть запуск/усиление рекламы.")
        lines.append("**ЭФФЕКТ:** Увеличение переходов на 20% через рекламу → дополнительные заказы и выручка.")
        lines.append("")
    ...
    if problem_articles and rec_num < 4:
        ...
        lines.append("**ГИПОТЕЗА:** Проблемы с контентом, нехватка размеров, негативные отзывы, или проблемы с поставками (нет в наличии на FBO).")
        lines.append("**ДЕЙСТВИЕ:** Проверить наличие на складах по проблемным артикулам. Полный аудит контента: фото, видео, описание, отзывы. Отключить рекламу на артикулы без остатков.")
        lines.append("**ЭФФЕКТ:** Снижение потерь, восстановление заказов по проблемным артикулам.")
        lines.append("")
