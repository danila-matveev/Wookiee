...
async def _run_report_pipeline(
    ...
    if not report_data.get("detailed_report") and compiler_result:
        ...
        if raw_output and len(raw_output) > 500:
            ...
            text_to_parse = fence_match.group(1).strip() if fence_match else raw_output

            # Try JSON parse (first { to last })
            first_brace = text_to_parse.find("{")
            last_brace = text_to_parse.rfind("}")
            if first_brace >= 0 and last_brace > first_brace:
                try:
                    parsed = json.loads(text_to_parse[first_brace:last_brace + 1])
                    if isinstance(parsed, dict) and parsed.get("detailed_report"):
                        report_data = parsed
                        logger.info(
                            "Recovered report from raw_output (%d chars detailed_report)",
                            len(parsed["detailed_report"]),
                        )
                except (json.JSONDecodeError, Exception) as e:
                    logger.warning("Failed to parse JSON from raw_output: %s", e)

            # Last resort: use raw markdown only if it starts with ## ▶ (not JSON)
            if not report_data.get("detailed_report"):
                clean = raw_output.strip()
                if clean.startswith("## ▶"):
                    report_data = {"detailed_report": clean, "telegram_summary": ""}
                    logger.info("Using raw_output as detailed_report (%d chars)", len(clean))
