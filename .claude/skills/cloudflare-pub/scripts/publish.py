...
def _is_pipe_table_row(line):
    """Check if line looks like a markdown pipe table row."""
    s = line.strip()
...
def _parse_pipe_row(line):
    """Split pipe table row into cells."""
    s = line.strip()
    if s.startswith("|"):
        s = s[1:]
    if s.endswith("|"):
        ...
        if _is_pipe_table_row(line):
            ...
            while i < len(lines) and _is_pipe_table_row(lines[i]):
                if not _is_separator_row(lines[i]):
                    table_rows.append(_parse_pipe_row(lines[i]))
                i += 1
            if table_rows:
                blocks.append(("table", table_rows))
            continue
...
def render_html(blocks, title=None):
    ...
    for btype, content in blocks:
        if btype in ("h1", "h2", "h3"):
            parts.append(f"<{btype}>{inline_md(content)}</{btype}>")

        elif btype == "p":
            text = inline_md(content)
            ...
            parts.append(f"<blockquote>{inline_md(content)}</blockquote>")
            ...
            for ri, row in enumerate(rows):
                ...
                parts.append("<tr>")
                for cell in row:
                    parts.append(f"<{tag}>{inline_md(cell)}</{tag}>")
                parts.append("</tr>")
...
code{{background:var(--stripe);padding:.15em .4em;border-radius:4px;font-size:.9em}}
blockquote{{
  margin:1rem 0;padding:.8rem 1.2rem;
  border-left:3px solid var(--accent);
  color:var(--muted);font-style:italic;
  background:var(--stripe);border-radius:0 6px 6px 0
}}
.footer{{
