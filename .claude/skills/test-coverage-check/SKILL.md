...
Skill копирует ignore/deselect-список из `.github/workflows/ci.yml` (telemost_recorder, product_matrix_api, и т.д.). Если CI меняется, обновить `CI_IGNORES` / `CI_DESELECTS` в `scripts/nightly/test_coverage_check.py`.
...
| Что | Что делает |
|---|---|
| `coverage` падает (нет файла, ошибка установки) | Exit 1, JSON с `truncated=true` и `blocking=true`, summary включает `error` |
| Все тесты падают | Coverage = 0%, blocking finding, exit 1 |
| Baseline отсутствует | Создаётся с текущим значением, finding не пишется |

## Контракт с night-coordinator

Coordinator читает `.hygiene/reports/coverage-YYYY-MM-DD.json`. Если `blocking=true` — **PR не мерджит**, на ветке стоит label `do-not-merge` (выставляет coordinator).
