...
from oleg_bot.services.agent_executor import AgentExecutor, AgentResult
from oleg_bot.services.agent_tools import TOOL_DEFINITIONS
from oleg_bot.services.json_utils import extract_json
...
class OlegAgent:
    ...
    def __init__(
        self,
        zai_client,
        playbook_path: str = "oleg_bot/playbook.md",
...
→ Конкретно: «Драйверы: Wendy (+46 тыс), Ruby (+25 тыс). Анти-драйверы: Joy WB (убыток -5.9 тыс, ДРР 41%)»
...
→ Если реклама снизилась — ПРОВЕРЬ: упали ли показы и переходы? Это TRADE-OFF!
   «Реклама -20% → показы -25% → заказы -16%. Прибыль на единицу выросла, но объём упал.»
→ НЕЛЬЗЯ рекомендовать «сократить рекламу ещё» если трафик уже падает
...
СТРУКТУРА brief_summary (СТРОГО):
[b]Сводка за [дата]:[/b]

[b]Что произошло:[/b]
1-2 предложения: главный факт + trade-off. «Маржа выросла на X% (+Y тыс), при этом заказы упали на Z%.»

[b]Почему:[/b]
2-3 предложения: причины с факторами. «Рост средней цены на X% и сокращение рекламы на Y%. Это увеличило прибыль на единицу, но привело к падению показов (-Z%).»

[b]Какие модели повлияли:[/b]
• Драйверы прибыли: Model1 (+X тыс маржи), Model2 (+Y тыс).
• Анти-драйверы: Model3 (убыток -X тыс, ДРР Y%), Model4 (ушла в минус).

[b]Что делать:[/b]
1. Конкретное действие с моделью/каналом.
...
- ОБЯЗАТЕЛЬНО: средняя цена/ед, показы (ad_views), переходы (card_opens)
