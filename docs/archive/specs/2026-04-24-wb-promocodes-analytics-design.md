...
### 3.1 Размещение на сервере

**Не создаём новый контейнер.** Добавляем роуты в существующий `wb-logistics-api` (он уже работает как Sheets+GAS hub для `wb-logistics-optimizer`). Контейнер де-факто становится мульти-роутером для всех Sheets+GAS-кнопок; имя оставляем (избегаем breaking change в существующем GAS).

```
services/wb_logistics_api/
    app.py                          # + новые роуты:
                                    #     POST /promocodes/run
                                    #     GET  /promocodes/status
                                    # (существующие /run, /status, /health — без изменений)
...
Ядро (`sync_promocodes.py`) вызывается из обоих путей: и из `app.py` (фоновый поток после POST /promocodes/run), и из CLI (cron / отладка). Состояние (`_state`) разделяется по namespace: `state.logistics`, `state.promocodes` — чтобы не путать.

### 3.2 Триггеры

| Триггер | Путь | Когда срабатывает |
|---|---|---|
| Cron на хосте Timeweb (системный crontab) | `curl -X POST http://localhost:8092/promocodes/run -H "X-API-Key: ..."` | вторник 09:00 МСК |
| GAS-кнопка «Обновить» | `UrlFetchApp` → `POST /promocodes/run` | по нажатию пользователя |
| Ручной запуск | `docker exec wb-logistics-api python scripts/run_wb_promocodes_sync.py` | отладка, бэкфилл |
...
> Все promocodes-роуты — под префиксом `/promocodes/*`. Существующие роуты `/run`, `/status`, `/health` остаются за логистикой (без изменений, без breaking change для текущего GAS).

### `POST /promocodes/run`

**Headers:** `X-API-Key: <token>` (отдельный `PROMOCODES_API_KEY`, не путаем с `WB_LOGISTICS_API_KEY`)
...
Существующий healthcheck контейнера — расширять не нужно. Здоровье promocodes-роута проверяем через `GET /promocodes/status`.

## 6. Деплой

**Новых docker-сервисов не создаём.** Всё живёт в существующем контейнере **`wb-logistics-api`**:

1. Добавляем код (`services/sheets_sync/sync/sync_promocodes.py`, новые роуты в `services/wb_logistics_api/app.py`, CLI-обёртку, env-переменные).
2. Передеплой:
   ```bash
   ssh timeweb
   cd /app && git pull
   ...
   ```
3. Cron на хосте Timeweb (системный `crontab -e`):
   ```cron
   0 9 * * 2  curl -sS -X POST http://localhost:8092/promocodes/run \
              -H "X-API-Key: $PROMOCODES_API_KEY" \
              -H "Content-Type: application/json" \
              -d '{"mode":"last_week"}' \
              >> /var/log/wb-promocodes-cron.log 2>&1
   ```
   Cron живёт на хосте, не внутри контейнера — единое место для всех расписаний, легко аудитить через `crontab -l`.

**Порт `8092`** — текущий порт `wb-logistics-api` (фиксируем явно в спеке, чтобы не путать).
...
```python
{
    ...
    "how_it_works": (
        ...
        "Триггеры: cron + GAS-кнопка → POST /promocodes/run"
    ),
    "status": "active",
    "version": "1.0.0",
    "run_command": "POST /promocodes/run | python scripts/run_wb_promocodes_sync.py --mode last_week",
...
- **Количество переходов / клики по товару с промокодом** — в `reportDetailByPeriod` v5 такого поля нет (это финансовый отчёт). Если интерпретировать «переходы» как «количество использований промокода», то это покрыто полем **Заказов, шт** (`orders_count`). Если нужны именно UI-клики по карточке — потребуется отдельный источник (`seller-analytics-api` per-keyword отчёт), и его не получится привязать к UUID промокода → отдельный план.

## 12. Открытые вопросы

Все закрыты по итогам обсуждения:
- ✅ Сервер: **переиспользуем `wb-logistics-api`** (новый контейнер не нужен), добавляем роуты `/promocodes/*`
- ✅ Cron: на хосте Timeweb через системный crontab, без cron внутри контейнера
- ✅ Дашборд: шапка в листе с timestamp + статусом + метриками
- ✅ Кнопка: GAS Drawing → POST `/promocodes/run`
