...
Перед запуском скилл уточняет период у пользователя. По умолчанию — **прошлая неделя** (текущая может быть незакрытой). Можно указать любую дату внутри нужной недели (`YYYY-MM-DD`) — сборщики возьмут всю неделю.
...
- **Раздел 2** — основная проблема недели (AI, с цифрами)
- **Раздел 3** — рекомендуемое решение (AI, с ответственным и сроком)
- **Раздел 6** — 16 моделей с выручкой, маржой, ДРР, трендом, рекомендуемым действием
- **Раздел 7** — внешняя реклама по каналам (блогеры, ВК, WB внутр.; Яндекс и посевы подрядчика — ручной ввод)
- **Раздел 8** — логистика: индекс локализации + оборачиваемость по всем 16 моделям
