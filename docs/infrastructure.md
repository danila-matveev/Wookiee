...
| `wookiee_oleg` | Олег — финансовый AI-агент (единый контейнер: bot + agent + scheduler) | — |
| `wookiee_sheets_sync` | Синхронизация Google Sheets | — |
| `vasily-api` | HTTP trigger для WB localization | — |
...
# oleg bot flow (/start) — вручную в Telegram
...
На этот сервер **ничего не деплоится**. Он используется как источник данных для аналитики (Олег, Sheets Sync, ETL).
