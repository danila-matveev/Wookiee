...
> **Дата:** 2026-02-25 (обновлено 2026-02-26 — верификация уникальных значений)
...
| Источник | Строк | Уникальных записей |
|----------|-------|--------------------|
| Листы Google Sheets (всего) | 36+ | — |
| Листы с данными каталога | 7 | — |
| Таблицы Supabase | 16 | — |
| Views Supabase | 9 | — |
| Строки в "Все модели" | 212 | 22 модели основы, ~40 моделей |
| Строки в "Все товары" | 4 788 | **1 452 уник. баркодов**, 479 уник. артикулов |
| Строки в "Все артикулы" | 193 | НЕ ИСПОЛЬЗУЕТСЯ (артикулы из "Все товары") |
| Строки в "Аналитики цветов" | 164 | 136 уник. color codes |
| Строки в "Упаковки" | 13 | — |
| Строки в "Склейки WB" | 556 | 27 уник. склеек |
| Строки в "Склейки Озон" | 1 406 | НЕ ИМПОРТИРУЕТСЯ |

> **Важно:** 3 336 строк (70%) в "Все товары" НЕ имеют баркода — это строки размеров, формулы, или незаполненные товары. Фактических дублей баркодов — 0.
...
### 4.1 Верифицированные уникальные значения (2026-02-26)

> Данные получены программным сравнением Google Sheets (gspread) и Supabase (execute_sql).

#### Справочники

| Измерение | Уник. в Sheets | В БД | Расхождение | Детали |
|-----------|---------------|------|-------------|--------|
| Категории | 7 | 3 | **+4 новых** | Леггинсы, Лонгслив, Рашгард, Футболка (новые, не в DB) |
| Коллекции | 8 | 7 | **+1 новая** | "Спортивная трикотажкая коллекция" |
| Импортёры | 2 | 2 | ✅ OK | — |
| Фабрики | 2 | 2 | ✅ OK | — |
| Размеры | 6 | 6 | ✅ OK | XS, S, M, L, XL, XXL |
| Статусы | 7 | 12 | **5 старых в DB** | В продаже, В разработке, Делаем образец, Закуп, Планирование (из MODEL_STATUS_MAP, не удалены) |
| Склейки WB | 27 | 27 | ✅ OK | — |
...
| Измерение | Уник. в Sheets | В БД | Расхождение | Детали |
|-----------|---------------|------|-------------|--------|
| Модели основы | 22 | 22 | ✅ OK | — |
| Модели (вариации) | ~40 | 40 | ✅ OK | 3 модели из "Все товары" не находятся через lookup (см. ниже) |
| Цвета (color_code) | 136 | 135 | **+1/-1** | WE021 нет в DB; "31" есть в DB но нет в Sheets |
| Артикулы | 479 | 478 | **-1** | "Wendy/yellow" — не импортируется (color WE021 не в каталоге) |
| Товары (баркоды) | 1 452 | 1 450 | **-2** | "#N/A" (невалидный) + "4670437808362" (Wendy/yellow) |

#### Модели основы по статусам товаров

| Статус | Кол-во уник. моделей основы |
|--------|----------------------------|
| Продается | 13 (Alice, Audrey, Bella, Joy, Lana, Moon, Ruby, Set Moon, Set Ruby, Set Vuki, Valery, Vuki, Wendy) |
| Новый | 12 |
| Выводим | 12 |
| Архив | 8 |
| План | 15 |
...
#### Артикулы по статусам

| Статус | Sheets | DB | Расхождение |
|--------|--------|-----|-------------|
| Продается | 153 | 133 | -20 |
| Новый | 39 | 40 | +1 |
| Выводим | 137 | 129 | -8 |
| Архив | 18 | 18 | 0 |
| Подготовка | 15 | 15 | 0 |
| План | 77 | 77 | 0 |
| Запуск | 40 | 66 | +26 |
| **Активные (Нов+Прод+Выв)** | **329** | **302** | **-27** |

> ⚠️ Значительные расхождения в "Продается" и "Запуск" — возможно, статусы обновлены в Sheets после миграции.

#### Товары по размерам

| Размер | Sheets (уник.) | DB |
|--------|---------------|-----|
| XS | 4 | 4 |
| S | 477 | 476 |
| M | 476 | 476 |
...
#### Анализ строк "Все товары" (4 788 строк)

| Категория | Кол-во |
|-----------|--------|
| Уникальных баркодов | 1 452 |
| Пустой баркод (строки без БАРКОД) | 3 336 |
| Невалидный баркод | 0 |
| Дублей баркодов | **0** |

> 70% строк (3 336 из 4 788) — строки БЕЗ баркода (строки размеров, формулы, пустые). **Дублей баркодов по юрлицам — нет.**

### 4.2 Верифицированные проблемы целостности (2026-02-26)

#### Артикулы без модели (`model_id = NULL`): 9 шт

| Артикул | Статус | Причина |
|---------|--------|---------|
| set_ruby/black | Продается | "set ruby" не находится через lookup (регистр!) |
| set_ruby/white_black_nude | Продается | ↑ |
| set_ruby/white | Продается | ↑ |
| set_ruby/latte | План | ↑ |
| set_ruby/spring | Запуск | ↑ |
| set_ruby/skin | Запуск | ↑ |
| Charlotte/brown | Запуск | "Charlotte" — новая модель, нет в "Все модели" |
| Charlotte/black | Запуск | ↑ |
| Charlotte/dark_beige | Запуск | ↑ |

**Причина:** В "Все товары" колонка "Модель" = `"set ruby"` (lowercase), но в DB модель записана как `"Set Ruby"`. Lookup `self._modeli.get("set ruby")` → None. Аналогично "Charlotte" — модель существует в "Все товары", но нет в "Все модели".

#### Артикулы без цвета (`cvet_id = NULL`): 5 шт

| Артикул | Статус | Причина |
|---------|--------|---------|
| set_Wendy/nude | План | Color code "nude" не в "Аналитики цветов"? |
| Angelina/white | Выводим | Color code не найден |
| Angelina/black | Выводим | ↑ |
| Angelina/dark_red | Выводим | ↑ |
| Angelina/nude | Архив | ↑ |

#### Модели без основы (`model_osnova_id = NULL`): 1 шт

| Модель | Описание |
|--------|----------|
| Evelyn | Orphaned — в "Все модели" есть код "Evelyn", но "Модель основа" пуста или не совпадает |

#### Модели из "Все товары" не найденные через lookup: 3

| Значение "Модель" | Причина |
|-------------------|---------|
| `Charlotte` | Новая модель, нет в "Все модели" |
| `set ruby` | Регистр: в DB `"Set Ruby"`, в Sheets `"set ruby"` |
| `Дргуие товары` | Опечатка (не модель) |

#### Набор (nabor): все FALSE

Поле `modeli.nabor = FALSE` для всех 40 моделей, включая Set-модели (Set Vuki, Set Moon, Set Ruby, Set Wendy). Вероятно, столбец "Набор" в "Все модели" пуст или не парсится `clean_boolean()`.

#### tip_kollekcii: 21 из 22 заполнено

Вопреки ожиданиям (нет маппинга), поле заполнено (вероятно, миграцией 001):
- `tricot`: 10 моделей
- `seamless_wendy`: 10 моделей
- `seamless_audrey`: 1 модель
- `NULL`: 1 модель

Значения на английском; пользователь хочет русские названия.
...
2. **🔴 CRITICAL:** 9 артикулов без модели (set_ruby: регистр lookup, Charlotte: нет в "Все модели")
3. **⚠️ HIGH:** 4 новые категории + 1 коллекция не в DB (новые товары не импортируются)
4. **⚠️ HIGH:** 5 старых статусов не удалены из DB (В продаже, Закуп и др.)
5. **⚠️ HIGH:** `nabor` = FALSE для всех моделей, включая Set-модели
6. **⚠️ HIGH:** ~15 каталожных колонок отсутствуют в БД (сертификаты, компонентные SKU, товарный знак, Price CNY, статусы Сайт/Ламода)
7. **⚠️ HIGH:** Упаковки хранятся как строка, а не как FK на отдельную таблицу
8. **⚠️ MEDIUM:** 5 артикулов без цвета (Angelina, set_Wendy)
9. **⚠️ MEDIUM:** 1 orphaned модель "Evelyn" (нет model_osnova)
10. **⚠️ MEDIUM:** Распределение статусов различается между Sheets и DB (Продается: 153 vs 133)
11. **ℹ️ MEDIUM:** `v_matrica_cveta_modeli` — хардкод 11 из 22 моделей
12. **ℹ️ LOW:** Лист "Все артикулы" не используется — потенциальный источник путаницы
