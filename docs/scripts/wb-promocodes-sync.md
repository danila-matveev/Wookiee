...
2. **Скрипт каждую неделю** читает справочник, создаёт строки на листе аналитики
   (по одной на каждую пару `(UUID, кабинет с галочкой)`), фетчит метрики из WB
   API и обновляет ячейки. **Незнакомые промокоды** (UUID есть в API-ответе, но
   нет в справочнике) **тоже попадают в аналитику** — отдельной строкой с именем
   `неизвестный` (подсвечена жёлтым через CF). Их UUID параллельно пишутся в
   `result["unknown_uuids"]`, чтобы потом руками добавить в справочник.
