...
> Примечание: `abc_date.count_orders` = выкупленные позиции (продажи, не заказы!). Истинное число заказов — `COUNT(*)` из `orders`. GAS использует `orders` для заказов в рублях и `abc_date.count_orders` для шт. — наш код делает то же.
...
Та же таблица `RNP_EXT_ADS_SHEET_ID`, другой лист (название уточнить при реализации).
