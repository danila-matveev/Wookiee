...
### Кто запускает (cron-механизм)
**GitHub Actions scheduled workflow** — `.github/workflows/hygiene-daily.yml`:
- Cron: `0 3 * * *` UTC = 00:00 São Paulo (UTC-3, без DST в Бразилии с 2019).
- Job через `anthropics/claude-code-action@v1` (или прямой curl к Anthropic API) → запускает headless Claude session с промптом «Run /hygiene».
- Секреты в GitHub Secrets: `ANTHROPIC_API_KEY`, `TELEGRAM_MANAGER_BOT_TOKEN`, `HYGIENE_TELEGRAM_CHAT_ID`, `SUPABASE_*`, `CLOUDFLARE_API_TOKEN`.
- Concurrency-guard `cancel-in-progress: false` — не запускать второй раз пока первый идёт.
- Workflow timeout 30 мин (hard cap всего job'а).
- На GitHub Actions failure → встроенный notification в Watch repo settings.

**Почему не Claude `CronCreate`:** ограничение 7 дней авто-expiry + срабатывание только в idle REPL. Не подходит для production-routine 24/7.

**Почему не cron на App Server:** лишний уровень инфры, секреты пришлось бы дублировать, нужен ssh-доступ из CI. GitHub Actions — нативная среда репозитория, секреты централизованы.
...
```yaml
schedule:
  runner: github_actions     # см. .github/workflows/hygiene-daily.yml
  cron: "0 3 * * *"          # 03:00 UTC = 00:00 São Paulo (UTC-3, без DST)
...
notifications:
  ...
  telegram:
    bot_env: TELEGRAM_MANAGER_BOT_TOKEN     # переиспользуем @matveevos_head_bot (id=8792120003)
    chat_env: HYGIENE_TELEGRAM_CHAT_ID
    only_if_ask_user_or_security: true      # тихо, если только auto-fix
    message_prefix: "🧹 Wookiee Hygiene"     # дифференциация от остальных алертов того же бота
    message_format: |
      🧹 Wookiee Hygiene {date}
...
### 6.1 Telegram-бот для алертов
**Решение: переиспользуем существующий `@matveevos_head_bot` (id=8792120003, "Батя бот Matveev OS"), токен уже в `.env` как `TELEGRAM_MANAGER_BOT_TOKEN`.**

Обоснование:
- Стандартный Telegram Bot API не поддерживает «бот создаёт бота» (нет endpoint'а `bots.createBot` в Bot API — только в MTProto user-level, что требует phone session + TG_API_ID/HASH, оверкилл).
- Манагер-бот сейчас не имеет server-side логики (`getMyCommands` пуст) — это просто рабочий бот с токеном.
- Для hygiene-алертов нужно ровно одно: бот, который умеет slать сообщения в указанный chat_id. Один бот покрывает.
- Дифференциация сообщений от других возможных алертов того же бота — префикс `🧹 Wookiee Hygiene`.
- Если позже захочется отдельный child-бот — `/newbot` у @BotFather займёт 30 секунд, заменишь токен в env.

**Что нужно сделать единоразово:**
1. Получить `HYGIENE_TELEGRAM_CHAT_ID` — отправь `/start` боту `@matveevos_head_bot` со своего аккаунта (или из Wookiee-чата), затем:
   ```bash
   curl -s "https://api.telegram.org/bot$TELEGRAM_MANAGER_BOT_TOKEN/getUpdates" | jq '.result[-1].message.chat.id'
   ```
2. Добавить в `.env`: `HYGIENE_TELEGRAM_CHAT_ID=<число>`.
3. Реплицировать в GitHub Secrets (для CI): `TELEGRAM_MANAGER_BOT_TOKEN`, `HYGIENE_TELEGRAM_CHAT_ID`.

(Шаги 1-2 — implementation plan делает автоматически за тебя при первом setup-прогоне через `gh secret set`.)
...
7. `.env.example` — плейсхолдеры `TELEGRAM_MANAGER_BOT_TOKEN`, `HYGIENE_TELEGRAM_CHAT_ID`.
8. `.github/workflows/hygiene-daily.yml` — scheduled workflow (см. §6.3).
9. `.github/workflows/hygiene-config.md` — README для workflow с описанием секретов.

### 6.3 GitHub Actions scheduled workflow
Файл: `.github/workflows/hygiene-daily.yml` (создаётся implementation-плагом единоразово — recurring runs hygiene этот файл не трогают, см. protected_zones).
- `on.schedule.cron: "0 3 * * *"` (00:00 São Paulo).
- `on.workflow_dispatch:` (для ручного запуска без ожидания cron).
- Job:
  1. Checkout repo с full history (нужно для git-команд hygiene).
  2. Setup `gh` CLI + auth via `GITHUB_TOKEN`.
  3. Запуск Claude session через `anthropics/claude-code-action@v1` с промптом `/hygiene` и доступом к нужным MCP-серверам (Supabase, Cloudflare).
  4. Передать секреты как env vars: `ANTHROPIC_API_KEY`, `TELEGRAM_MANAGER_BOT_TOKEN`, `HYGIENE_TELEGRAM_CHAT_ID`, `SUPABASE_*`, `CLOUDFLARE_API_TOKEN`.
  5. Concurrency: один в момент времени.
  6. Timeout: 30 минут.
...
- Через `/tool-register` создать запись в таблице `tools`:
  - `name`: `hygiene`
  - `type`: `maintenance`
  - `schedule`: `daily 00:00 BRT (GitHub Actions)`
...
- Локально: `/hygiene` через Claude Code REPL — убедиться что флоу работает end-to-end.
- В CI: триггерить `workflow_dispatch` вручную из GitHub UI — убедиться что cron-окружение даёт тот же результат.
- Проверить: Cloudflare-статья создана, в `tool_runs` есть запись, если есть ask-user → Telegram пришёл от `@matveevos_head_bot` с префиксом `🧹 Wookiee Hygiene`.
- Если что-то не так — fix → повторный workflow_dispatch.
...
- `/hygiene` запускается локально через Claude Code REPL и завершается без ошибок.
- `.github/workflows/hygiene-daily.yml` зарегистрирован, секреты в GitHub Secrets, ручной `workflow_dispatch` проходит.
- Минимум один cron-запуск прошёл автоматически (наблюдается в `tool_runs` + GitHub Actions log).
- Cloudflare-статья создаётся каждый запуск.
- Telegram алерт от `@matveevos_head_bot` с префиксом `🧹 Wookiee Hygiene` приходит только при ask-user/security.
...
1. **`HYGIENE_TELEGRAM_CHAT_ID`** — узнаётся при первом setup-прогоне после `/start` боту `@matveevos_head_bot` от пользователя. Implementation plan делает это автоматически через `getUpdates`.
2. **Cloudflare Pages namespace** — куда именно публиковать. Решит skill `/cloudflare-pub` сам — он уже сконфигурирован.
...
4. **`anthropics/claude-code-action@v1`** — нужно проверить, что эта action существует и поддерживает все необходимые tools (Bash, Read, Edit, MCP). Implementation plan валидирует первой задачей; fallback — самописный bash-step с прямым curl к Anthropic API.
