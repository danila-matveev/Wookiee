...
Новый файл `wookiee-hub/src/components/layout/marketing-layout.tsx` (паттерн `catalog-layout.tsx`). Структура из JSX:773-776:
- `data-section="marketing"` атрибут на корневом `<div>` для активации stone-палитры
- Sub-sidebar `w-44` с заголовком «МАРКЕТИНГ» и двумя пунктами (`Промокоды` `Percent`, `Поисковые запросы` `Hash`)
- `<Outlet />` из `react-router-dom` для дочерних страниц

Регистрация в `router.tsx`: оборачиваем оба marketing-роута. Текущая регистрация — две отдельные lazy-страницы под `AppShell`; меняем на:
```tsx
...
- Pills «Канал»: `Все` + uniq из `channel_label` view (resolved label из `marketing.channels`, не slug)
...
**B.0.1** `database/marketing/views/2026-05-13-search-queries-unified-v2.sql` — **additive-апдейт** существующего view (сохраняем все колонки текущей версии + добавляем три новых: `entity_type`, `channel_label`, `sku_label`; `model_hint` улучшается через JOIN на `modeli_osnova`):
...
SELECT
  ...
  COALESCE(
    ...
    bq.canonical_brand
  )                                         AS model_hint,            -- IMPROVED: kod first, fallback to canonical_brand
  NULL::text                                AS sku_label,              -- NEW
  NULL::text                                AS creator_ref,
  (SELECT ch.label FROM marketing.channels ch WHERE ch.slug = 'brand') AS channel_label,  -- NEW
  bq.status                                 AS status,
...
SELECT
  ...
  (SELECT m.kod FROM public.modeli_osnova m WHERE m.id =
    (SELECT a.model_osnova_id FROM public.artikuly a WHERE a.id = sa.artikul_id)) AS model_hint,  -- IMPROVED
  (SELECT a.artikul FROM public.artikuly a WHERE a.id = sa.artikul_id) AS sku_label,  -- NEW
  sa.creator_ref                            AS creator_ref,
  (SELECT ch.label FROM marketing.channels ch WHERE ch.slug = sa.purpose) AS channel_label,  -- NEW
  sa.status                                 AS status,
...
**Изменения** vs текущий view (`2026-05-09-search-queries-unified.sql`):
- **NEW колонки:** `entity_type`, `sku_label`, `channel_label`
- **IMPROVED:** `model_hint` для brand — `kod FROM modeli_osnova` (fallback `canonical_brand`); для substitute — `kod` через JOIN на artikuly→modeli_osnova (раньше NULL)
- **Сохранено:** `source_id`, `source_table`, `security_invoker`, GRANT — критично для существующего фронта

`nomenklatura_wb` оставляем как text (текущий тип в таблице), не приводим к bigint.
...
2. `feat(marketing): use channel_label from view in pills + table cells (not slug). Depends on B.0.1 view update; before B.0.1 lands, frontend reads from useChannels() lookup`
...
- Catalog grouping pattern: `wookiee-hub/src/pages/catalog/matrix.tsx:60-63` (type+options), `:135` (getGroupKey), `:264` (grouped useMemo), `:381` (`<select>` render)
- ui_preferences helpers: `wookiee-hub/src/lib/catalog/service.ts:2452+` (getUiPref/setUiPref)
- Layout pattern reference: `wookiee-hub/src/components/layout/catalog-layout.tsx`
