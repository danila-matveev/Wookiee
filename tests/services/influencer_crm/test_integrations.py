...
def test_list_returns_page(client, auth):
    r = client.get("/api/integrations", headers=auth, params={"limit": 5})
...
def test_get_404(client, auth):
    r = client.get("/api/integrations/999999999", headers=auth)
...
def test_stage_transition(client, auth):
    list_resp = client.get("/api/integrations", headers=auth, params={"limit": 1, "stage_in": ["done"]}
    ).json()
    if not list_resp["items"]:
        import pytest
    ...
    r = client.post(f"/api/integrations/{iid}/stage",
        headers=auth,
    ...
    # restore
    client.post(f"/api/integrations/{iid}/stage",
        headers=auth,
