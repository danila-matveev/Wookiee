...
def test_report_error_includes_attempt():
    ...
    assert "2/3" in msg
    assert "дневной фин" in msg
    assert "подключиться" in msg  # humanized ConnectionError
...
def test_report_retries_exhausted():
    ...
    assert "дневной фин" in msg
    assert "вручную" in msg.lower() or "командой" in msg.lower()


def test_watchdog_alert_warning():
    msg = watchdog_alert("warning", ["db", "last_run"], ["llm"])
    assert "Проблемы" in msg
    assert "✗" in msg
    assert "✓" in msg
...
def test_report_exception():
    ...
    assert "timeout" in msg.lower() or "ожидания" in msg
...
def test_anomaly_report_basic():
    ...
    assert "аномали" in msg.lower()  # matches "аномалий" / "аномалии"
    assert "Критических отклонений: 1" in msg
    assert "Выручка" in msg
