...
> По умолчанию — прошлая неделя (пн {дата_пн} — вс {дата_вс}).

Дефолт — **прошлая неделя**, потому что текущая может быть незакрытой и данные по ней неполными. Если пользователь явно не указал период — покажи прошлую неделю и жди подтверждения или корректировки.

Получив ответ, определи `REF_DATE` — любую дату внутри нужной недели в формате `YYYY-MM-DD`. Например, если нужна неделя 28 апр – 4 мая, `REF_DATE=2026-04-28`. Без аргумента сборщики сами берут прошлую неделю.
